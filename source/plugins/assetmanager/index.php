...
if($bWriteFolderAdmin){
	...
	if(isset($_FILES["File1"]) && BASIC_USERS::init()->getPermission($assetManager->model->system_name, 'add')){
	...
	if(isset($_POST["inpFileToDelete"]) && BASIC_USERS::init()->getPermission($assetManager->model->system_name, 'delete')){
			...
			if($sItem=="."||$sItem=="..") {
				...
				if(!$fIsDirectory) {
					...
					if($bDisplay){
						...
						echo "<td><img src='".$root_manager_virtual."images/".$sIcon."'></td>";
						...
						if(BASIC_USERS::init()->getPermission($assetManager->model->system_name, 'delete')){
...
if(ref && (ref.oUtil || ref.tinyMCE)){
	...
	//document.write("<scr"+"ipt src='<?php print $root_manager_virtual; ?>language/"+oUtil.langDir+"/asset.js'></scr"+"ipt>");
</script>
...
<td valign=top style="background:url('bg.gif') no-repeat right bottom;padding-top:5px;padding-left:5px;padding-right:5px;padding-bottom:0px;">
		...
		<td>
				...
				<?php if(BASIC_USERS::init()->getPermission($assetManager->model->system_name, 'folders-manager')){?>
		...
		<?php if(BASIC_USERS::init()->getPermission($assetManager->model->system_name, 'add')){ ?>
		<tr>
		<td>
