<?php
/**
 * Form control for the global service "BASIC_GENERATOR". 
 * For more information abouit controls see cms/basic/generator.mod.php/BASIC_GENERATOR:registrateControle()
 * 
 * @author Evgeni Baldzhiyski
 * @version 1.3.3
 * @since 13.09.2012
 */
class TinyMCEControle extends BasicControl implements FormControlInterface{
	public  $manager = '';
	/**
	 * special parameters:
	...
	public function generate($name, $value, $attributes = array()) {
		...
		if(!isset($this->attributes['manager'])){
			$cmp = null;
			if($this->manager){
				$cmp = Builder::init()->build($this->manager);
			}else{
				$cmp = Builder::init()->getDisplayComponentBySystemName('AssetManager');
			}
			if($cmp){
				$this->attributes['manager'] = BASIC::init()->ini_get('root_virtual').BASIC::init()->dirName().BASIC::init()->scriptName().'?editor='.str_replace("plugin/", '', $cmp->manager);
			}else{
